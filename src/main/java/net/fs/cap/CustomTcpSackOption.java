...
public final class CustomTcpSackOption implements TcpOption {
   ...
   */
    ...
    private final List<Sack> sacks = new ArrayList<>();

    private CustomTcpSackOption(byte[] rawData, int offset, int length) throws IllegalRawDataException {
        if (length < 2) {
            String sb = "The raw data length must be more than 1. rawData: " +
        ...
        if ((lengthFieldAsInt - 2) % (INT_SIZE_IN_BYTES * 2) != 0) {
            String sb = "The value of length field must be an integer multiple of 8 octets long but: " +
                    lengthFieldAsInt;
            throw new IllegalRawDataException(sb);
        }
        if (length < lengthFieldAsInt) {
            String sb = "rawData is too short. length field: " +
                    lengthFieldAsInt +
                    ", rawData: " +
                    ByteArrays.toHexString(rawData, " ") +
                    ", offset: " +
